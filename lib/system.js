...
// ── ENV Builder "Apply Live" ──
// Only fields that Hermes reads from config.yaml (not bare process env at
// gateway-process startup) can take effect without a full container restart.
// Provider-credential vars like ANTHROPIC_API_KEY are derived once by start.sh
// at boot and aren't re-read here — those still require a Space restart.
const LIVE_CONFIG_KEYS = [
  ...
  "CUSTOM_API_KEY",
  "CUSTOM_PROVIDER",
  "CUSTOM_MODEL_CONTEXT_LENGTH",
  "CUSTOM_MODEL_MAX_TOKENS",
  "HERMES_BACKGROUND_NOTIFICATIONS",
];

function applyEnvLive(fields) {
  const applied = LIVE_CONFIG_KEYS.filter((key) => Object.prototype.hasOwnProperty.call(fields, key));
  const skipped = Object.keys(fields).filter((key) => !LIVE_CONFIG_KEYS.includes(key));
...
def env(name):
    return os.environ.get(f"HM_LIVE_{name}")

m = config.setdefault("model", {})

model_name = env("LLM_MODEL")
if model_name is not None:
    m["default"] = model_name.strip()

provider_name = env("HERMES_INFERENCE_PROVIDER")
...
custom_base = env("CUSTOM_BASE_URL")
if custom_base is not None:
    custom_base = custom_base.strip()
    if custom_base:
        m["base_url"] = custom_base.rstrip("/")
    else:
        m.pop("base_url", None)

custom_provider = env("CUSTOM_PROVIDER")
if custom_provider is not None and custom_provider.strip():
    m["provider"] = custom_provider.strip()

api_key = env("CUSTOM_API_KEY") or env("LLM_API_KEY")
if api_key:
    m["api_key"] = api_key

for env_name, cfg_key in (("CUSTOM_MODEL_CONTEXT_LENGTH", "context_length"), ("CUSTOM_MODEL_MAX_TOKENS", "max_tokens")):
    raw = env(env_name)
    if raw is not None and raw.strip():
        try:
            m[cfg_key] = int(raw.strip())
        except ValueError:
            pass

notif = env("HERMES_BACKGROUND_NOTIFICATIONS")
if notif is not None and notif.strip():
    config.setdefault("display", {})["background_process_notifications"] = notif.strip()
...
`;
  ...
  for (const key of applied) {
    env[`HM_LIVE_${key}`] = fields[key] == null ? "" : String(fields[key]);
  }
  execFileSync("python3", ["-c", script], { env, timeout: 5000 });
