...
(node "$APP_DIR/health-server.js" 2>&1 | tee -a "$HERMES_HOME/logs/health.log") &
HEALTH_PID=$!
...
start_background_sync_once() {
  ...
  (python3 -u "$APP_DIR/hermes-sync.py" loop 2>&1 | tee -a "$HERMES_HOME/logs/sync.log") &
  SYNC_LOOP_PID=$!
}
...
while true; do
  ...
  # Honor a pause request from the dashboard's gateway "Stop" control — block
  # here instead of relaunching until the pause file is removed ("Start").
  GATEWAY_PAUSE_FILE="$HERMES_HOME/.gateway.paused"
  if [ -f "$GATEWAY_PAUSE_FILE" ]; then
    echo "Gateway paused via dashboard; waiting..."
    while [ -f "$GATEWAY_PAUSE_FILE" ]; do
      sleep 2
    done
    echo "Gateway unpaused; resuming."
  fi
